...
public class HunterTest {
    ...
    public void setUp() {
        ...
        h = tileFactory.producePlayer(6);
        h.initialize(p1,messageCallback);
    ...
    public void abilityCast() {
        h.abilityCast(abilityCastCallback);
    ...
    private boolean abilityCastCheck() {
        ...
        boolean e1_hurt = e1_damage > 0,
                e2_hurt = e2_damage > 0;
        boolean e1_valid_damage = e1_damage <= 30 && e1_damage >= 27;
        ...
        // validHit is true if and only if the closest enemy got hit with a valid damage
        boolean validHit = e1_hurt &&  e1_valid_damage && !e2_hurt;
    ...
    @Test
    public void levelUp() {
        int previousHealth = h.getHealth().getPool();
        int previousAttack = h.getAttack();
        int previousDefense = h.getDefense();
        int healthBonus = 20;
        int attackBonus = 12;
        int defenseBonus = 4;
        h.levelUp();

        assertEquals(2, h.getPlayerLevel());
        assertTrue(h.getHealth().getAmount() == h.getHealth().getPool() && h.getHealth().getAmount() == previousHealth + healthBonus);
        assertTrue(h.getAttack() == previousAttack + attackBonus);
        assertTrue(h.getDefense() == previousDefense + defenseBonus);

        previousHealth = h.getHealth().getPool();
        previousAttack = h.getAttack();
        previousDefense = h.getDefense();
        healthBonus = 30;
        attackBonus = 18;
        defenseBonus = 6;
        h.levelUp();

        assertEquals(3, h.getPlayerLevel());
        assertTrue(h.getHealth().getAmount() == h.getHealth().getPool() && h.getHealth().getAmount() == previousHealth + healthBonus);
        assertTrue(h.getAttack() == previousAttack + attackBonus);
        assertTrue(h.getDefense() == previousDefense + defenseBonus);
    }
    ...
    public void onGameTick() {
        ...
        Empty e = new Empty(p);
        h.onGameTick('d',(c)->e,abilityCastCallback);
        assertEquals(p, h.getPosition());
        assertEquals(e.getPosition(),previousPosition);

        // Ability test:
        h.onGameTick('e',moveUnitCallback,abilityCastCallback);
        assertTrue(abilityCastCheck());
    }
}